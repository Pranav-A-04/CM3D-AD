    ...
    def get_statistics(self):
        ...
        if len(self.cates) == len(all_shapenetad_cates):
            ...
            stats_save_path = os.path.join(
                stats_dir, "stats_" + "_".join(self.cates) + ".pt"
            )
            ...
            if self.split == "train":
                ...
                for pc_id in tqdm(range(self.num_aug), "Augment"):
                    ...
                    if random.random() < 0.8:
                        ...
                        patch_scale = 0.2
                        pointcloud_aug, mask_aug = random_patch(
                            pointcloud, int(patch_num), patch_scale
                        )
                ...
                for pc_id, f in enumerate(os.listdir(local_path)):
                    if "positive" in f:
                        ...
                        choice = np.random.choice(
                            len(pointcloud), self.num_points, False
                        )
                        ...
                        local_file = os.path.join(
                            local_path.replace("test", "GT"), f.replace("pcd", "txt")
                        )
                        pointcloud_mask = np.genfromtxt(
                            local_file, dtype=np.float32, delimiter=","
                        )
