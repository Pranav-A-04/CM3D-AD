...
def validate_loss(it):
    ...
    roc_i, roc_p, ap_i, ap_p = metrics['ROC_i'], metrics['ROC_p'], metrics['AP_i'], metrics['AP_p']
    logger.info('[Val] Iter %04d | ROC_i_cdist %.6f | ROC_p_cdist %.6f | AP_i_cdist %.6f | AP_p_cdist %.6f' % (it, roc_i, roc_p, ap_i, ap_p))
    roc_i_nn, roc_p_nn, ap_i_nn, ap_p_nn = metrics['ROC_i_nn'], metrics['ROC_p_nn'], metrics['AP_i_nn'], metrics['AP_p_nn']
    ...
    while it <= args.max_iters:
        ...
        if it % args.val_freq == 0 or it == args.max_iters:
            ...
            if it ==args.max_iters:
                ...
                ckpt_mgr.save(model, args, score, opt_states, it, is_best=True,iter=args.max_iters)
