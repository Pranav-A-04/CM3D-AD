...
def load_point_cloud(file_path, num_points=2048):
    ...
    scale_info = {"center": center, "scale": scale}
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["consistency", "diffusion"],
        help="Model type to use for inference",
    )
    parser.add_argument(
        "--pointcloud", type=str, required=True, help="Path to input point cloud (.pcd)"
    )
    parser.add_argument(
        "--num_points",
        type=int,
    ...
    with torch.no_grad():
        ...
        trajectory = model.decode(
            points, code, points.size(1), flexibility=args.flexibility, ret_traj=False
        )
